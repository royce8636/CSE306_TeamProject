...
void func(int client_socket, int file, int addr)
{
    ...
    while (1)
    {
        ...
        case 0:
            printf("Arrow up\n");
            int data1[4] = {1, 80, 1, 80};
            write_i2c_block_data(file, addr, reg, 4, data1);
            break;
        case 1:
            printf("Arrow down\n");
            int data2[4] = {0, 80, 0, 80};
            write_i2c_block_data(file, addr, reg, 4, data2);
            break;
        case 2:
            printf("Arrow right\n");
            int data3[4] = {1, 30, 1, 80};
            write_i2c_block_data(file, addr, reg, 4, data3);
            break;
        case 3:
            printf("Arrow left\n");
            int data4[4] = {1, 80, 1, 30};
            write_i2c_block_data(file, addr, reg, 4, data4);
            break;
        case 4:
            printf("Stop\n");
            int data5[4] = {0, 0, 0, 0};
            write_i2c_block_data(file, addr, reg, 4, data5);
            break;
        }
    }
}
